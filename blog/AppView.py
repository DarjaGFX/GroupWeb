...
def Narlogin(request):
    ...
    if views.is_Email_format(email):
        ...
        if views.is_Email_format(email):
            if views.is_Email_used(email):
                ...
                new_member = members.objects.create(
                    user = new_user ,
                    Token = views.CreateToken() )
                try:
                    img = UploadProPicForm(request.POST, request.FILES)
                    if img.is_valid():
                        uniqueID = views.CreateToken()
        ...
        if str(user[0].user.groups.all()[0].name) == 'admin':
            if name !="" and desc != "":
                ...
                if created:
                    ...
                    return JsonResponse({'Status':'0x0015'} ,encoder=JSONEncoder)
                ngr.save()
                return JsonResponse({'Status':'0x0000'} ,encoder=JSONEncoder)
            else:
                return JsonResponse({'Status':'0x0006'} ,encoder=JSONEncoder)
    ...
    if len(user)>0:
        if user[0].user.is_active:
            ...
            try:
                ...
                if not views.is_Email_format(newEmail.lower()):
                    tmp = {'Email':'0x0006'}
                    arr.append(tmp)
                else:
                    if newEmail != u.email :
                        if views.is_Email_used(newEmail.lower()):
                            tmp = {'Email':'0x0002'}
                            arr.append(tmp)
                        else:
                            code = views.CreateToken()
...
def MailAvailability(request):
    ...
    if views.is_Email_format(arg.lower()):
        if views.is_Email_used(arg.lower()):
            return JsonResponse({'Status':'0x0002'},encoder=JSONEncoder)
        else:
            return JsonResponse({'Status':'0x0000'},encoder=JSONEncoder)
    else:
...
def forget_pass_request(request):
    ...
    if views.is_Email_format(mail.lower()):
        if views.is_Email_used(mail.lower()):
            ...
            if not len(user)>0:
                ...
                u = user[0]
                code = views.CreateToken()[:4].upper()
...
def resend_veriffication_mail(request):
    mail = request.POST['email']
    mail = mail.lower()
    if views.is_Email_format(mail):
        if views.is_Email_used(mail):
